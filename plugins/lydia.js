...
Morfin.addCommand({pattern: 'addlydia$', fromMe: true, desc: Lang.ADDLYDIA_DESC}, (async (message, match) => {
...
Morfin.addCommand({pattern: 'rmlydia$', fromMe: true, desc: Lang.RMLYDIA_DESC}, (async (message, match) => {
    if (!message.reply_message) return await message.reply(Lang.NEED_REPLY);
    if (!Config.COFFEEHOUSE_API_KEY) return await message.reply(Lang.COFFEEHOUSE);
    var unix = Date.now() / 1000 | 0;
...
Morfin.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {   
